  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50/30">
      <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
        ...
        {/* The Dashboard Component with Analytics Tab default */}
        <PortfolioDashboard assets={assets} error={error} fetchTime={fetchTime} />
        
        {/* Fallback & Debugging Section */}
        <div className="mt-12 p-8 bg-white rounded-3xl border border-slate-200 shadow-sm">
          ...
          {assets.filter(a => a.group !== "Crypto").length === 0 && (
            <p className="text-slate-400 font-medium text-center py-10">No stock assets found in your sheet.</p>
          )}
        </div>
      </div>
    </div>
